...
const PillNav: React.FC<PillNavProps> = ({
  ...
  return (
    <div className="absolute top-[1em] z-[1000] w-full left-0 md:w-auto md:left-auto">
      ...
      <div
        ref={mobileMenuRef}
        className="md:hidden fixed top-[3em] left-4 right-4 rounded-[27px] shadow-[0_8px_32px_rgba(0,0,0,0.12)] z-[998] origin-top"
        style={{
          ...cssVars,
        ...
        <ul className="list-none m-0 p-[3px] flex flex-col gap-[3px]">
          {items.map((item) => {
            ...
            return (
              <li key={item.href}>
                {isRouterLink(item.href) ? (
                  <Link to={item.href} className={linkClasses} style={defaultStyle} onMouseEnter={hoverIn} onMouseLeave={hoverOut} onClick={() => setIsMobileMenuOpen(false)}>
                    {item.label}
                  </Link>
                ) : (
